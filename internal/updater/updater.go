...
func (u *githubReleaseUpdater) UpdateFrom(currentVersion string) (string, error) {
	...
	if strings.TrimPrefix(latestResponse.TagName, "v") == strings.TrimPrefix(currentVersion, "v") {
		return fmt.Sprintf("keychain-parser is already at latest version %s", latestResponse.TagName), nil
	}
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				return "", fmt.Errorf("invalid archive from %s, no keychain-parser binary found", matchingAsset.Url)
			}

			return "", fmt.Errorf("failed to read archive file: %v", err)
		}

		if hdr.Name == "keychain-parser" {
			...
			latestBinary, err := os.CreateTemp(originalPath, fmt.Sprintf("keychain-parser-%s", latestResponse.TagName))
			if err != nil {
